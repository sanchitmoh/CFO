...
def _compute_forecast_prophet(
    ...
    for i in range(months_ahead):
        ...
        # Confidence based on uncertainty interval width
        income_uncertainty = (income_upper - income_lower) / max(proj_income, 1)
        expense_uncertainty = (expense_upper - expense_lower) / max(proj_expense, 1)
        avg_uncertainty = (income_uncertainty + expense_uncertainty) / 2
        
        # Convert uncertainty to confidence (inverse relationship)
        confidence = max(0.5, min(1.0, 1.0 - (avg_uncertainty / 2)))
        
        # Confidence margin for display
...
class ProphetForecastService:
    ...
    async def generate_forecast(
        ...
        cache_key = await make_versioned_cache_key(
            "forecast_prophet", str(workspace_id), scenario, str(months_ahead)
        )
        ...
        db_result = await db.execute(
            ...
            .where(and_(
                ...
                ForecastResult.model_version == "v1_prophet",
            ))
